...
public:
    ...
    Array& operator=(const Array& other) {
        if (this != &other) {
            delete[] _data; // Удаляем старый массив
            _size = other._size;
            _capacity = other._capacity;
            _data = new std::shared_ptr<T>[other._capacity]; // Создаем новый массив
            for (size_t i = 0; i < _size; ++i) {
                _data[i] = other._data[i];
            }
        }
    ...
    void push_back(const std::shared_ptr<T>& value) {
        if (_size >= _capacity) {
            resize(_capacity == 0 ? 1 : _capacity * 2); // Увеличиваем размер массива
        }
        _data[_size++] = value;
    }
    ...
    void resize(size_t new_capacity) {
        if (new_capacity > _capacity) {
            ...
            for (size_t i = 0; i < _size; ++i) {
                new_data[i] = std::move(_data[i]); // Используем перемещение
            }
            delete[] _data;
            _data = new_data;
    ...
    void remove(size_t index) {
        ...
        for (size_t i = index; i < _size - 1; ++i) {
            _data[i] = std::move(_data[i + 1]); // Используем перемещение
        }
        --_size;
    }
